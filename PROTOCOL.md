...
> - **v0.3** added a Registry transparency log so readers can verify
>   independently that published reviews are not silently deleted or
>   rewritten. Wire format incompatible with v0.2 (responses carry
>   `sth` + `inclusion_proof`).
> - **v0.4** drops `issued_at` from the credential message vector.
>   Revealing issuance time at publish time would let the issuer
>   correlate mint events with public reviews and weaken unlinkability;
>   key rotation / validity is expressed through the issuer's keyset
>   (`kid`) instead. Wire format incompatible with v0.3.
...
各 credential は以下のメッセージベクトル `(m₁, m₂, m₃, m₄)` への BBS+ 署名:
...
- `messages = [merchant_id, purchase_tier, product_category]` (issuer reveal)
- `disclosed_commitment_indexes = [0]` 固定 (hpk は nullifier として常に開示)
- `disclosed_indexes` ⊆ `[0, 1, 2]` で、最低 `[0]` (merchant_id) を含む
...
### なぜ `issued_at` を credential に持たないか

issuance time を credential 属性として持つと、 BBS+ proof 公開時に時刻も同時に出る (= 必須開示) か、 proof 内に scalar として残る (= 仕組みが複雑になる) かのどちらかになる。

前者は Issuer 側のmint log と publish 時刻のタイミング相関で reviewer を再特定できる余地を残し、G2 (Issuer-side unlinkability) を弱める。 credential 有効期間が必要なら issuer の keyset (`kid`) を周期的に rotate することで表現できるので、 値そのものを credential に埋め込まずに済む。
...
```
Inputs:  MintContext { merchant_id, purchase_tier, product_category }
...
7. Issuer resolves SK_m for merchant_id, computes blind signature blind_σ
   over (commitment, [merchant_id, purchase_tier, product_category])
8. Issuer returns blind_σ (along with PK_m, kid_m)
9. Reviewer unblinds: σ ← unblind(blind_σ, r)
10. Verify σ is valid BBS+ signature on (m₁, m₂, m₃, m₄) under PK_m
...
5. Generate fresh BBS+ blind proof:
       π = bbs_blind_proof_gen(
           signature: σ,
           messages:           [merchant_id, tier, category],
...
3. Reconstruct disclosure state from payload.credential_proof:
       disclosed_indexes        = [0]
                                  + [1] if purchase_tier is Some
                                  + [2] if product_category is Some
       disclosed_messages       = [merchant_id,
...
| 4 | Disclosure 戦略 | ✅ **v0.4**: `hpk` / `merchant_id` は必須開示、 `purchase_tier` / `product_category` は reviewer-controlled。 `issued_at` は credential に持たない (unlinkability 優先) |
